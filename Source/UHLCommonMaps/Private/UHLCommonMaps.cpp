...
#include "UHLCommonMaps.h"
#include "AssetSelection.h"
#include "UHLCommonMapsDeveloperSettings.h"
#include "FileHelpers.h"
#include "AssetRegistry/AssetRegistryModule.h"

#define LOCTEXT_NAMESPACE "FUHLCommonMapsModule"
...
namespace CommonMapsFunctionLibrary
{
	...
	static TSharedRef<SWidget> GetCommonMapsDropdown()
	{
		...
		for (const FUHLCommonMapCategory& MapCategory : GetDefault<UUHLCommonMapsDeveloperSettings>()->MapsCategories)
		{
	...
	{
		return HasNoPlayWorld() && !GetDefault<UUHLCommonMapsDeveloperSettings>()->MapsCategories.IsEmpty();
	}
...
void FUHLCommonMapsModule::ExtendContextMenu()
{
	...
	Section.AddSubMenu(FName("Common Maps"),
		...
		FNewMenuDelegate::CreateRaw(this, &FUHLCommonMapsModule::CreateCategorySelectionSubmenu),
		FToolUIActionChoice(),
...
void FUHLCommonMapsModule::AddToCommonMapsFromMenu(FName CategoryName)
{
	TArray<FAssetData> SelectedAssets;
	AssetSelectionUtils::GetSelectedAssets(SelectedAssets);
	if (auto* Settings = GetMutableDefault<UUHLCommonMapsDeveloperSettings>())
	{
		for (const auto& AssetData : SelectedAssets)
		{
			if (FUHLCommonMapCategory* CurrentCategory = Settings->MapsCategories.FindByKey(CategoryName))
			{
				CurrentCategory->Maps.Add({ AssetData.GetSoftObjectPath() });
			}
			else
			{
				Settings->MapsCategories.Add(FUHLCommonMapCategory(CategoryName, { AssetData.GetSoftObjectPath() }));
			}
		}
...
void FUHLCommonMapsModule::CreateCategorySelectionSubmenu(FMenuBuilder& MenuBuilder)
{
	const UUHLCommonMapsDeveloperSettings* DevSettings = GetDefault<UUHLCommonMapsDeveloperSettings>();
	if (!DevSettings) return;

	for (const FUHLCommonMapCategory& Category : DevSettings->MapsCategories)
	{
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(), FUIAction(FExecuteAction::CreateRaw(this, &FUHLCommonMapsModule::AddToCommonMapsFromMenu, Category.Name)));
	}
}


void FUHLCommonMapsModule::ShutdownModule()
{
...
IMPLEMENT_MODULE(FUHLCommonMapsModule, UHLCommonMaps)
